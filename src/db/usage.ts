...
export function recordSessionStart(sessionId: string, apiKey: string | undefined, opts: {
  proxyUrl?: string;
  stealthLevel?: string;
  ...
  db.prepare(`
    ...
    VALUES (?, ?, 'active', ?, ?, ?, ?, ?, ?)
  `).run(sessionId, key, now, opts.proxyUrl ?? null, opts.stealthLevel ?? null,
    opts.viewportWidth ?? 1280, opts.viewportHeight ?? 900, opts.profileId ?? null);
...
export function recordSessionEnd(sessionId: string, browserHours: number) {
  ...
  const session = db.prepare('SELECT api_key, created_at FROM sessions WHERE id = ?').get(sessionId) as any;
  if (!session) return;
...
export function recordApiCall(apiKey: string | undefined, method: string, path: string, statusCode: number, durationMs: number) {
  const db = getDb();
  const key = apiKey ?? 'anonymous';
...
export function getUsageStats(apiKey?: string): UsageStats {
  ...
  const totalSessions = (db.prepare(
    'SELECT count(*) as c FROM sessions WHERE api_key = ?'
  ).get(key) as any)?.c ?? 0;

  const activeSessions = (db.prepare(
    'SELECT count(*) as c FROM sessions WHERE api_key = ? AND status = ?'
  ).get(key, 'active') as any)?.c ?? 0;

  const totalBrowserHours = (db.prepare(
    'SELECT COALESCE(SUM(browser_hours), 0) as h FROM sessions WHERE api_key = ?'
  ).get(key) as any)?.h ?? 0;

  const today = new Date().toISOString().slice(0, 10);
  const todayRow = db.prepare(
    'SELECT browser_hours, api_calls FROM daily_usage WHERE api_key = ? AND date = ?'
