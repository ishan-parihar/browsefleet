...
export function captchaRoutes(pool: BrowserPool): Hono {
  ...
  app.post('/:id/captcha/solve', async (c) => {
    ...
    if (!detected) {
      return c.json({ success: false, type: 'none', duration: Date.now() - start, error: 'No CAPTCHA detected on page' } satisfies CaptchaSolveResponse);
    }
    ...
    try {
      ...
      if (createData.status !== 1) {
        return c.json({ success: false, type: captchaType, duration: Date.now() - start, error: `2captcha error: ${createData.request}` } satisfies CaptchaSolveResponse);
      }
      ...
      for (let i = 0; i < 24; i++) {
        ...
        if (resultData.request !== 'CAPCHA_NOT_READY') {
          return c.json({ success: false, type: captchaType, duration: Date.now() - start, error: `2captcha error: ${resultData.request}` } satisfies CaptchaSolveResponse);
        }
      }

      if (!solution) {
        return c.json({ success: false, type: captchaType, duration: Date.now() - start, error: 'CAPTCHA solve timed out' } satisfies CaptchaSolveResponse);
      }

      // Inject solution
      await page.evaluate((sol: string, type: string) => {
        if (type === 'recaptcha') {
          const textarea = document.querySelector('#g-recaptcha-response') as HTMLTextAreaElement;
          if (textarea) { textarea.value = sol; textarea.style.display = 'block'; }
          // Trigger callback
      ...
      logger.info({ sessionId: session.id, captchaType, duration: Date.now() - start }, 'CAPTCHA solved');
      return c.json({ success: true, type: captchaType, duration: Date.now() - start } satisfies CaptchaSolveResponse);
    } catch (err: any) {
      return c.json({ success: false, type: captchaType, duration: Date.now() - start, error: err.message } satisfies CaptchaSolveResponse);
    }
  });
